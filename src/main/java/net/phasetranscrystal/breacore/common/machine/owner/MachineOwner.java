package net.phasetranscrystal.breacore.common.machine.owner;
...
public abstract sealed class MachineOwner permits PlayerOwner {
    ...
    public static boolean canOpenOwnerMachine(Player player, MetaMachine machine) {
        ...
        if (player.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.byId(ConfigHolder.INSTANCE.machines.ownerOPBypass)))) return true;
    ...
    public static void displayPlayerInfo(List<Component> compList, UUID playerUUID) {
        ...
        var online = "brealib.tooltip.status.trinary.";
        if (BreaLib.isClientThread()) {
