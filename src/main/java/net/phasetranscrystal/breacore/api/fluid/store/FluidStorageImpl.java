...
public class FluidStorageImpl implements FluidStorage {
    ...
    private Map<FluidStorageKey, FluidRegisterBuilder> toRegister = new Object2ObjectOpenHashMap<>();
     ...
     * @param builder the FluidRegisterBuilder to build
     */
    @Override
    ...
    /**
     * @param key the key corresponding with the FluidRegisterBuilder
     ...
     */
    @ApiStatus.Internal
    public void registerFluids(@NotNull Material material, BreaRegistrate registrate) {
        if (registered) {
        ...
        if (toRegister.isEmpty() && map.isEmpty()) {
            enqueueRegistration(FluidStorageKeys.LIQUID, new FluidRegisterBuilder());
        }
        ...
        toRegister.entrySet().stream()
                ...
                .forEach(entry -> {
                    if (map.containsKey(entry.getKey())) {
                        BreakdownCore.LOGGER.warn("{} already has an associated fluid for material {}", entry.getKey(),
                                material);
                        return;
                    }
                    ...
                    if (!storeNoOverwrites(entry.getKey(), fluid, entry.getValue())) {
                        BreakdownCore.LOGGER.error("{} already has an associated fluid for material {}", material, material);
                    }
                });
