 ...
 * 在实体对目标造成伤害时触发的适配事件。
 *
 * <p>此事件转发自伤害序列相关事件，用于将伤害事件适配到实体事件分发系统。
 *
 * <p><b>事件转发链：</b>
 * <ul>
...
public abstract class EntityAttackEvent extends EntityEvent {
     ...
     * 伤害收入事件。
     *
     * <p>转发自 {@link LivingIncomingDamageEvent}，事件链中最早的可取消点。
     * 可用于拦截或修改即将造成的伤害。
     *
     ...
     * 伤害计算前事件。
     *
     * <p>转发自 {@link LivingDamageEvent.Pre}，在伤害值计算之前触发。
     * 可用于在最终伤害计算前执行额外逻辑。
     *
     ...
     * 伤害计算后事件。
     *
     * <p>转发自 {@link LivingDamageEvent.Post}，在伤害值计算之后触发。
